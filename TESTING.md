# Goosker LevelZ Overhaul 0.3.33-alpha
...
0.3.32-alpha Smithing furnace pass:
- Adds Smithing-scaled furnace fuel efficiency: -1% fuel consumption per Smithing level, reaching -50% at level 50.
- Adds Smithing bonus XP to every cooking recipe with vanilla cooking XP, reaching 250% total at Smithing 50.
- Ore XP recognizes vanilla copper/iron/gold/ancient debris, BetterEnd Thallasium ore/raw recipes found in the supplied jar, and conventionally named modded ore/raw smelting/blasting recipes. Non-ore recycling/cooking recipes are excluded.
- Modded ore recipes that declare 0 furnace XP receive an iron-like 0.7 baseline only for this Smithing LevelZ-XP bonus; vanilla furnace XP is not changed.
- Smithing detail page adds Fuel Consumption (native furnace item render) and Smelting XP (existing Goosker XP icon) and briefly states that bonus XP applies only to ores.


0.3.33-alpha Smithing vanilla XP correction:
- Fixes Smelting XP so the Smithing multiplier now awards ordinary vanilla Minecraft experience instead of LevelZ skill XP.
- Qualifying ore recipes keep their normal vanilla furnace XP and gain +3% additional vanilla XP per Smithing level, reaching +150% bonus / 250% total at Smithing 50.
- Fuel Consumption, ore filtering, existing UI, textures, and all other gameplay values are unchanged from 0.3.32-alpha.
